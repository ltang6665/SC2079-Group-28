...
class StreamListener:
    ...
    def req_stream(self):
        ...
        header_txt = (header.decode("utf-8", errors="ignore").strip() if header else "")
        if header_txt != "OK STREAMING":
            raise RuntimeError(
                f"Stream handshake failed: '{header_txt or 'NO HEADER'}'. "
    ...
    def start_stream_read(self, on_result, on_disconnect, conf_threshold=0.7, show_video=True):
        ...
        try:
            ...
            logging.exception(f"Stream read loop failed: {e}")
