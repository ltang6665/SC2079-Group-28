...
from pathfinding.consts import Direction, EXPANDED_CELL, SCREENSHOT_COST
from pathfinding.helper import is_valid
    ...
    def get_view_state(self, retrying) -> List[CellState]:
        ...
        if self.direction == Direction.NORTH:
            if retrying == False:
                ...
                if is_valid(self.x + 1, self.y + 1 + EXPANDED_CELL * 2):
                    cells.append(CellState(self.x + 1, self.y + 1 + EXPANDED_CELL * 2, Direction.SOUTH, self.obstacle_id, SCREENSHOT_COST * 2))
                # Or (x - 1, y + 3)
                if is_valid(self.x - 1, self.y + 1 + EXPANDED_CELL * 2):
                    cells.append(CellState(self.x - 1, self.y + 1 + EXPANDED_CELL * 2, Direction.SOUTH, self.obstacle_id, SCREENSHOT_COST * 2))
        ...
        # If obstacle is facing south, then robot's cell state must be facing north
            ...
            if retrying == False:
                ...
                # Or (x + 1, y - 3)
                if is_valid(self.x + 1, self.y - 1 - EXPANDED_CELL * 2):
                    cells.append(CellState(self.x + 1, self.y - 1 - EXPANDED_CELL * 2, Direction.NORTH, self.obstacle_id, SCREENSHOT_COST * 2))
                # Or (x - 1, y - 3)
                if is_valid(self.x - 1, self.y - 1 - EXPANDED_CELL * 2):
                    cells.append(CellState(self.x - 1, self.y - 1 - EXPANDED_CELL * 2, Direction.NORTH, self.obstacle_id, SCREENSHOT_COST * 2))
        ...
        # If obstacle is facing east, then robot's cell state must be facing west
            ...
            if retrying == False:
                ...
                if is_valid(self.x + 1 + EXPANDED_CELL * 2, self.y + 1):
                    cells.append(CellState(self.x + 1 + EXPANDED_CELL * 2, self.y + 1, Direction.WEST, self.obstacle_id, SCREENSHOT_COST * 2))
                # Or (x + 3,y - 1)
                if is_valid(self.x + 1 + EXPANDED_CELL * 2, self.y - 1):
                    cells.append(CellState(self.x + 1 + EXPANDED_CELL * 2, self.y - 1, Direction.WEST, self.obstacle_id, SCREENSHOT_COST * 2))
        ...
        # If obstacle is facing west, then robot's cell state must be facing east
            ...
            if retrying == False:
                ...
                # Or (x - 3,y + 1)
                if is_valid(self.x - 1 - EXPANDED_CELL * 2, self.y + 1):
                    cells.append(CellState(self.x - 1 - EXPANDED_CELL * 2, self.y + 1, Direction.EAST, self.obstacle_id, SCREENSHOT_COST * 2))
                # Or (x - 3,y - 1)
                if is_valid(self.x - 1 - EXPANDED_CELL * 2, self.y - 1):
                    cells.append(CellState(self.x - 1 - EXPANDED_CELL * 2, self.y - 1, Direction.EAST, self.obstacle_id, SCREENSHOT_COST * 2))
