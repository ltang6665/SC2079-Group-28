...
#define ARC_BASE_PWM 4800 // 5400 / 5000       // base duty for the driven (outer) wheel
...
void oledTask(void const *argument)
{
  ...
  for (;;)
  {
    ...
    snprintf(line, sizeof(line), "Tgt: %d", (int)arc_target_angle);
    OLED_ShowString(0, 16, (int)line);
    ...
    //	        // IR L and R: PC0 and PC1

    //	        //PC1
    //	        snprintf(line, sizeof(line), "IR1:%d.%02uV %d", ir1_mV/1000, (ir1_mV%1000)/10, ir1_obs);
    //			OLED_ShowString(0, 32, (uint8_t*)line);
    //
    //			//PC0
    //	        snprintf(line, sizeof(line), "IR0:%d.%02uV %d", ir0_mV/1000, (ir0_mV%1000)/10, ir0_obs);
    //	        OLED_ShowString(0, 48, (uint8_t*)line);

    snprintf(line, sizeof(line), "IR0:%d  IR1:%d", ir0_obs, ir1_obs);
    OLED_ShowString(0, 48, (uint8_t *)line);
  ...
  for (;;)
  {
    ...
    case CMD_ARC_LEFT:
    {
          ...
          (uart_cmd == CMD_ARC_LEFT && total_angle >= arc_target_angle))
      {
        ...
        // send_ack_one_cmd();  // COMMENTED OUT (Ryan told me to)
      ...
      if (uart_cmd == CMD_ARC_RIGHT)
      {
        ...
        if (aerr < 45.0)
          outer = (int)(1.00f * ARC_BASE_PWM); // 1.05
        if (aerr < 20.0)
      ...
      else
      {
        ...
        int outer = ARC_BASE_PWM;
        if (aerr < 45.0)
          outer = (int)(1.0f * ARC_BASE_PWM);
        if (aerr < 20.0)
          outer = (int)(1.0f * ARC_BASE_PWM);
        if (outer > ARC_MIN_DUTY)
          outer = ARC_MIN_DUTY;
        right_forward_duty(outer * 1.07); // 1.07, 1.00
