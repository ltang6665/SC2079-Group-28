...
static void send_ack(void) {
  const char msg[] = "OK\n";
  cmdq_clear();
  HAL_UART_Transmit(&huart3, (uint8_t*)msg, sizeof(msg)-1, 20);
}
...
#define PWM_MAX 7199
#define PWM_RUN 5600  // your current "run" duty
#define PWM_MIN 6800
#define PWM_INNER 6000

/* --- Gyro turn target & tuning --- */
volatile double arc_target_angle = 0.0;
#define ARC_DEG_TOL     2.5f          // stop when |error| < this many degrees
...
#define INTER_CMD_MS  300        // tweak 80–200ms, is the delay for inbetween commands
...
//keep uart isr shorter
static volatile bool line_ready = false;


/* --- Command timeout watchdog --- */
volatile uint32_t cmd_start_tick = 0;

/* Tunable: max time you’ll allow any single command to run */
#define CMD_TIMEOUT_MS  7000
  ...
  while (tok) {
    ...
    if (it.type != SCMD_NONE) {
      ...
      if (!ok) {
        ...
        OLED_ShowString(0, 32, (uint8_t*)emsg);
...
static void request_resend() {
    char buf[48];
    int n = snprintf(buf, sizeof(buf), "RESEND\n");
...
int main(void)
{
  ...
  osThreadDef(Motor_Task, motorTask, osPriorityAboveNormal, 0, 256);
...
void HCSR04_Trigger(void) {
    ...
    osDelay(1); // Short delay for 10µs pulse
    HAL_GPIO_WritePin(US_Trig_GPIO_Port, US_Trig_Pin, GPIO_PIN_RESET);
}
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART3) {
        // clear error by restarting reception
        HAL_UART_Receive_IT(&huart3, (uint8_t*)&rx_byte, 1);
    }
}
  ...
  /* Infinite loop */
	char line[32];
	    ...
	    for(;;)
	    {
	        ...
	        // Line 1: Yaw
	        snprintf(line, sizeof(line), "Yaw: %d", (int)total_angle);
  ...
  /* Infinite loop */
    for (;;)
    {
	  ...
	  if (abort_now) {
		 ...
		 arc_postforward_cm = 0;
		 const char k[] = "RST\r\n";
		 HAL_UART_Transmit(&huart3, (uint8_t*)k, sizeof(k)-1, 0xFFFF);
		 osDelay(5);
		 continue;   // skip normal control for this tick
	   }

	  /* ---- Command timeout watchdog ---- */
	  if (uart_cmd != CMD_NONE && cmd_start_tick != 0) {
	      uint32_t now = HAL_GetTick();
	      if ((now - cmd_start_tick) > CMD_TIMEOUT_MS) {
	          // 1) Safe stop
	          motor_brake();
	          set_servo_center();
	          obstacle_stop_mode = 0;

	          // 2) Clear state & queue so a resend starts cleanly
	          cmdq_clear();
	          uart_cmd = CMD_NONE;
	          next_start_tick = 0;
	          arc_postforward_cm = 0;
	          ...
	          // 3) Tell host to resend the last script line
	          request_resend();
...
//          const int   base_pwm = PWM_RUN;     // e.g., 4000
          const float Kp = 0.003f;	//0.05 original 0.003, 0.003, 0.000
          ...
          if (uart_cmd == CMD_FORWARD && obstacle_stop_mode) {
        	  ...
        	  done_by_ultra = (echo_debug > 5 && echo_debug <= OBSTACLE_STOP_CM);
          }
          ...
          if (done_by_counts || done_by_ultra) {
              ...
              cmd_start_tick = 0;   // disarm timeout
              uart_cmd = CMD_NONE;
...
//          int32_t err = (cpsA - cpsB);
          ...
          //GYRO
			...
			const float K_SERVO = 1.9f;        // was 3.5f; lower = less twitchy

			// Deadband: ignore tiny heading error (< 0.3°) to prevent servo hunting
			if (error_angle > -0.3f && error_angle < 0.3f) {
				error_angle = 0.0;
			}
			...
			int servo = SERVO_CENTER_CCR;

			if (uart_cmd == CMD_FORWARD) {
					servo = SERVO_CENTER_CCR - corr;   // left is smaller CCR for your servo map
				} else { // CMD_REVERSE
					servo = SERVO_CENTER_CCR + corr;   // flip sense in reverse
				}
          ...
          if (uart_cmd == CMD_FORWARD) {
              ...
              __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, lDuty); //Scale
              __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, PWM_MAX);
              // Right motor (TIM9: CH1=IN2, CH2=IN1)
              __HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_1, rDuty); //Removed 0.97 scale
              __HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_2, PWM_MAX);
          } else { // CMD_REVERSE
              __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, PWM_MAX);
              __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, lDuty);
              __HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_1, PWM_MAX);
              __HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_2, rDuty); //Removed 0.975 scale
          }
		...
		case CMD_ARC_LEFT:
		  {
			  ...
			  // --- Gyro error ---
			  double err  = arc_target_angle - total_angle;     // + => need more CW
...
//					  uart_cmd = CMD_FORWARD;                // hand over to distance controller
...
//				  }
			  ...
			  if (uart_cmd == CMD_ARC_RIGHT) {
				  ...
				  if (aerr < 45.0) outer = (int)(1.00f * ARC_BASE_PWM); //1.05
				  if (aerr < 20.0) outer = (int)(1.00f * ARC_BASE_PWM); //1.1
...
//				  right_coast(); // coast turning
				  right_forward_duty(inner*1.2); // best turn 30x11.5 tried 1.15, 1.2
...
//				  left_coast();
				  left_forward_duty(inner * 1.2); // best turn 30x10.5 1.15
