...
class Task1:
                ...
                if len(self.pending_obstacles) > 0:
                    ...
                    if max_obstacle_id is not None:
                        ...
                        if self.should_stitch and len(self.ids_to_stitch) >= self.stitch_len:
                            ...
                            self.should_stitch = False
                            stitch_images(self.ids_to_stitch, self.stitching_img_dict, filename='task1')
    ...
    def _segment_commands(self, commands):
        ...
        for cmd in commands:
            ...
            if cmd.startswith("SNAP"):
                ...
                snap_no.append(index)
                continue
            segments.append(cmd)
            index += 1
        return {"obstacle_ids": obstacle_ids, "segments": segments, "snap_no": snap_no}
    ...
    def snap_directions(self, full_path):
        """
        For each 'SNAP' command in `commands`, record the most recent direction
        from the last consumed `path` entry. SNAP/FIN do not consume a path entry.
        """
        path = full_path['path'][1:] if full_path else []
        commands = full_path['commands']
        snaps = []
        i_path = 0
        curr_dir = None
        ...
        for cmd in commands:
            if cmd.startswith("SNAP"):
                # Use the current direction (from the last non-SNAP command's path)
                snaps.append(self.direction_to_name(curr_dir))
            elif cmd == "FIN":
                # No path consumed
                continue
            else:
                # Non-SNAP command: consume the next path entry if available
                if i_path < len(path):
                    d = path[i_path].get("d")
                    curr_dir = d
                    i_path += 1
                # If path exhausted, keep last known direction
    ...
    def test_pathfinding(self):
        ...
        print(path)
        # snap = self.snap_directions(path)
        # print(snap)
