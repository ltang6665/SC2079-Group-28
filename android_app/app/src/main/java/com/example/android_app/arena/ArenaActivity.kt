...
/**
 * The main working screen: arena canvas + movement controls + status log.
 *
 * Checklist:
 *   C.3  interactive movement via labelled dpad buttons
 *   C.4  status TextView showing SELECTIVE messages (parsed events, not raw stream)
 *   C.5  hosts [ArenaView]
 *   C.6  emits OBSTACLE/OBSTACLE_DEL on placement/drag/delete
 *   C.7  face-picker dialog on long-press, emits FACE,<num>,<dir>
 *   C.9  handles inbound TARGET,<obs>,<id>
 *   C.10 handles inbound ROBOT,<x>,<y>,<dir>
 */
